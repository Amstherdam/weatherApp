import getWeather from "./fetchValue";
import {
  formatTime,
  wildConvert,
  capitalize,
...
import fallImg from "./img/fall.jpg";

//  import my img
const myImg1 = new Image();
myImg1.src = winterImg;
const myImg2 = new Image();
myImg2.src = fallImg;

const getValueInput = document.getElementById("get-location");

let getValueInputData;
let data;
...
let weather_location_time;
let weather_details_feelsLike;
let weather_details_humidity;
let weather_details_chanceOfRain;
let weather_details_windSpeed;
let weather_solo_icon;
let weather_daily_data;
//
let weather_sunrise;
let weather_sunset;
let changeBgTime;
...
function changeDOM() {
  ...
  getValueInput.addEventListener("keyup", async (e) => {
    if (e.key == "Enter" && getValueInputData != "") {
      whenSearchLocation();
    }
  });
}

/*  whenSearchLocation manipulates the DOM based on API data (main function)
 */
async function whenSearchLocation() {
  if (getValueInput.value === null || getValueInput.value === "") {
    getValueInputData = "amsterdam";
  } else {
    getValueInputData = getValueInput.value;
  }

  data = await getWeather(getValueInputData);

  if (data != undefined) {
    weather_description = data.weatherInformationData1.weather[0].description;
    weather_name = data.weatherInformationData1.name;
    weather_temperature = data.weatherInformationData2.current.temp;
    weather_location_time = data.weatherInformationData2.current.dt;
    timezoneoff = data.weatherInformationData2.timezone_offset;
    weather_sunrise = data.weatherInformationData2.current.sunrise;
    weather_sunset = data.weatherInformationData2.current.sunset;
    weather_solo_icon = data.weatherInformationData1.weather[0].icon;
    weather_daily_data = data.weatherInformationData2.daily;

    timeValue = formatTime(timezoneoff);

    theTime = timeValue.theTime;
    clock = timeValue.clock;
    theHour = timeValue.theHour;
    realHour = timeValue.realHour;

    weather_details_feelsLike = data.weatherInformationData2.current.feels_like;
    weather_details_humidity =
      data.weatherInformationData2.current.humidity + "%";
    weather_details_chanceOfRain =
      parseInt(data.weatherInformationData2.hourly[theHour].pop * 100) + "%";
    weather_details_windSpeed = wildConvert(
      data.weatherInformationData2.current.wind_speed
    );

    // sunset sunrise change bg image
    changeBgTime = setAndRise(weather_sunrise, weather_sunset, timezoneoff);

    //change icon function
    iconNestToDom(weather_solo_icon);

    // this function change to bg-image and text
    bgChangeForSunTime(changeBgTime, timeValue);

    //change daily temperature data
    dailyTemperature(weather_daily_data, timezoneoff);

    console.log(data.weatherInformationData1, data.weatherInformationData2);
    domManipulation(
      weather_description,
      weather_name,
      theTime,
      clock,
      weather_temperature,
      weather_details_feelsLike,
      weather_details_humidity,
      weather_details_chanceOfRain,
      weather_details_windSpeed
    );
  } else {
    const error = document.querySelector(".weather-search-box-error");
    error.style.visibility = "visible";
    error.innerHTML = ` Location not found. <br />
...
// DOM manipulated API data
function domManipulation(...vals) {
  
  const description = document.querySelector(".weather-info-description");
  const location = document.querySelector(".weather-location");
  const theDate = document.querySelector(".weather-date");
  const timex = document.querySelector(".weather-time");
  const temper = document.querySelector(".weather-temperature");
  const detail_feelsLike = document.getElementById("feels-like");
  const detail_humidity = document.getElementById("humidity");
  const detail_changeOfRain = document.getElementById("changeOfRain");
  const detail_windSpeed = document.getElementById("windSpeed");
  const minimalLocationName = document.getElementById("minimalLocationName");

  detail_feelsLike.innerHTML = "<span>°C</span>";
  temper.innerHTML = "<span>°C</span>";

  description.innerHTML = capitalize(vals[0]);
  location.innerHTML = vals[1];
  theDate.innerHTML = vals[2];
  timex.innerHTML = vals[3];
  temper.innerHTML = Math.round(vals[4]) + temper.innerHTML;
  detail_feelsLike.innerHTML = Math.round(vals[5]) + detail_feelsLike.innerHTML;
  detail_humidity.innerHTML = vals[6];
  detail_changeOfRain.innerHTML = vals[7];
  detail_windSpeed.innerHTML = Math.round(vals[8] * 10) / 10 + "km/h";
  minimalLocationName.innerHTML = vals[1];
}

/* bgChangeForSunTime change background image and some DOM manipulation

  param1: changeBgTime = an array have to locatıons sunset ,and sunrise
  param2  timeValue = current time of location
*/
function bgChangeForSunTime(changeBgTime, timeValue) {
  const body = document.getElementsByTagName("body")[0];
  const mainClass = document.querySelector(".main");
  const weatherSvg = document.querySelectorAll(".weatherSvg");
  ...
  if (
    !(
      changeBgTime.sunriseTime <= timeValue.realHour &&
      timeValue.realHour <= changeBgTime.sunsetTime
    )
  ) {
    mainClass.style.color = "whitesmoke";
    body.style.backgroundImage = `url( ${myImg1.src})`;
    serachInput.style.color = "whitesmoke";
    serachInput.style.borderBottomColor = "whitesmoke";
    changeBtn.style.borderColor = "whitesmoke";
    weatherSvg.forEach((weatherSvgItem) => {
      weatherSvgItem.style.fill = "whitesmoke ";
    });
  } else {
    body.style.backgroundImage = `url( ${myImg2.src})`;
    mainClass.style.color = "#060606";
    serachInput.style.color = "#060606";
    serachInput.style.borderBottomColor = "#060606";
    iconDiv.childNodes[0].style.fill = "#060606";
    changeBtn.style.borderColor = "#060606";
    weatherSvg.forEach((weatherSvgItem) => {
      weatherSvgItem.style.fill = "#060606 ";
    });
  }
}

/* this function change left side svg by weatherApi weather code

  param codeValue = weatherApi => weather code
 */
function iconNestToDom(codeValue) {
  const iconDiv = document.querySelector(".weather-icon");
  let iconSVG = getIconByCode(codeValue);
  iconDiv.innerHTML = iconSVG;
}

/*  dailyTemperature  This section adjusts the daily weather forecasts at the bottom of the page.

    dailyData: it is comes from API`s daily temp data

    timeZoneOffset : offset time for locations
*/
function dailyTemperature(dailyData, timeZoneOffset) {
  let domArr = document.querySelectorAll(".foracat-day-by-day");
  let maxTempDOMArr = [];
  let minTempDOMArr = [];
  let dayNameDOMArr = [];
  let dayWeatherIcon = [];
  ...
  let maxTempValueArr = [];
  let minTempValueArr = [];
  let dailyWeatherIconCodeArr = [];

  let unixTimeDaily = [];
  let currentTimeDaily = [];

  domArr.forEach((DomItem) => {
    dayNameDOMArr.push(DomItem.children[0]);
    maxTempDOMArr.push(DomItem.children[1].children[0]);
    minTempDOMArr.push(DomItem.children[1].children[1]);
    dayWeatherIcon.push(DomItem.children[2]);
  });

  dailyData.forEach((value) => {
    maxTempValueArr.push(Math.round(value.temp.max));
    minTempValueArr.push(Math.round(value.temp.min));
    dailyWeatherIconCodeArr.push(value.weather[0].icon);
    unixTimeDaily.push(value.dt);
  });

  maxTempValueArr.shift();
  minTempValueArr.shift();
  dailyWeatherIconCodeArr.shift();
  unixTimeDaily.shift();

  unixTimeDaily.forEach((itemUnix) => {
    currentTimeDaily.push(dailyForecastTime(itemUnix, timeZoneOffset));
  });

  for (let i = 0; i < 7; i++) {
    maxTempDOMArr[i].innerHTML = "<span>°C</span>";
    minTempDOMArr[i].innerHTML = "<span>°C</span>";

    dayNameDOMArr[i].innerHTML = currentTimeDaily[i];
    maxTempDOMArr[i].innerHTML =
      maxTempValueArr[i] + maxTempDOMArr[i].innerHTML;
    minTempDOMArr[i].innerHTML =
      minTempValueArr[i] + minTempDOMArr[i].innerHTML;
    dayWeatherIcon[i].innerHTML = getIconByCode(dailyWeatherIconCodeArr[i]);
  }
}

/*  covert to celcıus and fahrenheit

*/
let btn = document.querySelector(".changeBtn");

btn.addEventListener("click", () => {
  let tempConvertItem = document.querySelectorAll(".tempConvert");

  tempConvertItem.forEach((item) => {
    if (item.children[0].innerHTML == "°C") {
      let arrC = [];
      arrC.push(...item.textContent);
      arrC.pop();
      arrC.pop();
      let tempNumber = arrC.join("");
      let result = celsiusFahrenheit(tempNumber);
      item.innerHTML = result + "<span>°F</span>";
    } else {
      let arrF = [];
      arrF.push(...item.textContent);
      arrF.pop();
      arrF.pop();
      let tempNumber2 = arrF.join("");
      let result2 = fahrenheitCelsius(tempNumber2);
      item.innerHTML = result2 + "<span>°C</span>";
    }
  });
});


/* this function add svg to DOM by weather code

param code = weatherApi => weather code
